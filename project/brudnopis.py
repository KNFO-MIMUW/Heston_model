...
def integrate_char_function(theta, K, t, S_0, r, i, M, n, rule):
    ...
    if rule == "Riemann":
        ...
        for j in range(1, n+1):
            value = value + cmath.exp(-1j*j*delta_t*math.log(K/S_0))/(1j*j*delta_t)*char_function(theta, j*delta_t-i, t, S_0, r)
        value = value*delta_t
    elif rule == "Simpson":
        value = cmath.exp(-1j*delta_t*math.log(K/S_0))/(1j*delta_t)*char_function(theta, delta_t-i, t, S_0, r)
        for j in range(2, n, 2):
            value = value + 4*cmath.exp(-1j*j*delta_t*math.log(K/S_0))/(1j*j*delta_t)*char_function(theta, j*delta_t - i, t, S_0, r)
        for j in range(3, n-1, 2):
            value = value + 2*cmath.exp(-1j*j*delta_t*math.log(K/S_0))/(1j*j*delta_t)*char_function(theta, j*delta_t - i, t, S_0, r)
        value = value + cmath.exp(-1j*M*math.log(K/S_0))/(1j*M)*char_function(theta, M - i, t, S_0, r)
...
def eur_call_heston_price(theta, K, t, S_0, r, M, n, rule):
    return (S_0 - math.exp(-r*t)*K)/2 + math.exp(-r*t)/math.pi*(S_0*integrate_char_function(theta, K, t, S_0, r, 1j, M, n, rule) - \
                                                                K*integrate_char_function(theta, K, t, S_0, r, 0, M, n, rule))
def eur_call_heston_price_GL(theta, K, t, S_0, r, M, deg):
    return (S_0 - math.exp(-r*t)*K)/2 + math.exp(-r*t)/math.pi*(S_0*integrate_char_function_GL(theta, K, t, S_0, r, 1j, M, deg) - \
                                                                K*integrate_char_function_GL(theta, K, t, S_0, r, 0, M, deg))

theta = np.array([0.09, 0.09, -0.3, 2, 1])
#print(eur_call_heston_price(theta, 100, 5, 100, 0.05, 10, 100000, "Riemann"))
...
def r_function(theta, mkt_data, S_0, r, M, deg):
    r_vector = np.array(eur_call_heston_price_GL(theta, mkt_data[0, 1], mkt_data[0, 2], S_0, r, M, deg) - \
                        mkt_data[0, 0])
    for i in range(1, len(mkt_data)):
        r_vector = np.append(r_vector, eur_call_heston_price_GL(theta, mkt_data[i, 1], mkt_data[i, 2], S_0, r, M, deg) - \
                             mkt_data[i, 0])
    return r_vector
...
def J_matrix(theta, mkt_data, S_0, r, M, deg):
    J = grad_heston_price(theta, mkt_data[0, 2], mkt_data[0, 1], S_0, r, M, deg)
    J.shape = (5, 1)
    for i in range(1, len(mkt_data)):
        J_next = grad_heston_price(theta, mkt_data[i, 2], mkt_data[i, 1], S_0, r, M, deg)
        J_next.shape = (5, 1)
...
def calibration_LM(theta, mkt_data, omega, eps1, eps2, eps3, S_0, r, M, deg):
    k = 0
    J = J_matrix(theta, mkt_data, S_0, r, M, deg)
    norm_r_theta = norm_r(theta, mkt_data, S_0, r, M, deg)
    mu = omega*np.dot(np.transpose(J),J).diagonal().max() # tutaj nie wiem do konca czym jest diag(J), gdy macierz nie jest kwadratowa
    v = 2
    while 1 == 1:
        k = k+1
        ...
        if (delta_L > 0 and delta_F > 0): # Wyznaczyc funcje na delta_L oraz delta_F
            J = J_matrix(theta_next, mkt_data, S_0, r, M, deg)
            theta = theta_next
        else:
            mu = mu*v
            v = 2*v
        if (norm_r_theta <= eps1 or np.dot(np.transpose(J), np.ones(5)).max() <= eps2 or np.dot(
                np.transpose(delta_theta), delta_theta) / np.dot(np.transpose(theta), theta) <= eps3):
            return theta

theta = np.array([0.08, 0.10, -0.8, 3, 0.25])
mkt_data = np.vstack(([eur_call_heston_price_GL(theta, 80, 0.25, 100, 0.02, 100, 64), 80, 0.25],
                      [eur_call_heston_price_GL(theta, 80, 0.5, 100, 0.02, 100, 64), 80, 0.5],
                      [eur_call_heston_price_GL(theta, 80, 1, 100, 0.02, 100, 64), 80, 1],
                      [eur_call_heston_price_GL(theta, 80, 2, 100, 0.02, 100, 64), 80, 2],
                      [eur_call_heston_price_GL(theta, 90, 0.25, 100, 0.02, 100, 64), 90, 0.25],
                      [eur_call_heston_price_GL(theta, 90, 0.5, 100, 0.02, 100, 64), 90, 0.5],
                      [eur_call_heston_price_GL(theta, 90, 1, 100, 0.02, 100, 64), 90, 1],
                      [eur_call_heston_price_GL(theta, 90, 2, 100, 0.02, 100, 64), 90, 2],
                      [eur_call_heston_price_GL(theta, 100, 0.25, 100, 0.02, 100, 64), 100, 0.25],
                      [eur_call_heston_price_GL(theta, 100, 0.5, 100, 0.02, 100, 64), 100, 0.5],
                      [eur_call_heston_price_GL(theta, 100, 1, 100, 0.02, 100, 64), 100, 1],
                      [eur_call_heston_price_GL(theta, 100, 2, 100, 0.02, 100, 64), 100, 2],
                      [eur_call_heston_price_GL(theta, 110, 0.25, 100, 0.02, 100, 64), 110, 0.25],
                      [eur_call_heston_price_GL(theta, 110, 0.5, 100, 0.02, 100, 64), 110, 0.5],
                      [eur_call_heston_price_GL(theta, 110, 1, 100, 0.02, 100, 64), 110, 1],
                      [eur_call_heston_price_GL(theta, 110, 2, 100, 0.02, 100, 64), 110, 2],
                      [eur_call_heston_price_GL(theta, 120, 0.25, 100, 0.02, 100, 64), 120, 0.25],
                      [eur_call_heston_price_GL(theta, 120, 0.5, 100, 0.02, 100, 64), 120, 0.5],
                      [eur_call_heston_price_GL(theta, 120, 1, 100, 0.02, 100, 64), 120, 1],
                      [eur_call_heston_price_GL(theta, 120, 2, 100, 0.02, 100, 64), 120, 2],
                      ))
#np.array([0.08, 0.10, -0.8, 3, 0.25])
theta_0 = np.array([0.07, 0.11, -0.75, 3.15, 0.15])
#print(calibration_LM(theta_0, mkt_data, 1, 0.0000000001, 0.0000000001, 0.0000000001, 100, 0.02, 50, 10))
S_0 = 100
r = 0.02
M = 100
deg = 64
def r_function_test(theta):
    r_vector = np.array(eur_call_heston_price_GL(theta, mkt_data[0, 1], mkt_data[0, 2], S_0, r, M, deg) - \
                        mkt_data[0, 0])
    for i in range(1, len(mkt_data)):
        r_vector = np.append(r_vector, eur_call_heston_price_GL(theta, mkt_data[i, 1], mkt_data[i, 2], S_0, r, M, deg) - \
...
print(optimize.least_squares(r_function_test, theta_0, method='lm').x)
