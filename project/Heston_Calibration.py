# Equation (18) shows the characteristic function, which we are going to use in out project
...
def char_function(theta, u, t, S_0, r):
    return cmath.exp(1j*u*(math.log(S_0)+r*t)-t*theta[3]*theta[1]*theta[2]*1j*u/theta[4]-theta[0]*A(theta,u,t)+2*theta[3]*theta[1]*D(theta,u,t)/math.pow(theta[4],2))
...
def eur_call_heston_price(theta, K, t, S_0, r, M, n, rule):
    return (S_0-math.exp(-r*t)*K)/2+math.exp(-r*t)/math.pi*(integrate_char_function(theta,K,t,S_0,r,1j,M,n,rule)-K*integrate_char_function(theta,K,t,S_0,r,0,M,n,rule))
def eur_call_heston_price_GL(theta, K, t, S_0, r, M, deg):
    return (S_0-math.exp(-r*t)*K)/2+math.exp(-r*t)/math.pi*(integrate_char_function_GL(theta,K,t,S_0,r,1j,M,deg)-K*integrate_char_function_GL(theta,K,t,S_0,r,0,M,deg))
